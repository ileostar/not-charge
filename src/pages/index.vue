...
import { budgets, loadBudgets, loadRecordsbymonth, recordsBymonth } from '@/API/loadRecords'
...
function updateCardInfo() {
  ...
  cardInfo.value[0].description = `比上个月${expensePercentageChange !== 'N/A' && Number.parseFloat(expensePercentageChange) >= 0 ? '多' : '少'}${Math.abs(Number.parseFloat(expensePercentageChange))}%`

  cardInfo.value[1].number = `+${thisMonthIncome.value.toFixed(2)}`
  cardInfo.value[1].description = `比上个月${incomePercentageChange !== 'N/A' && Number.parseFloat(incomePercentageChange) >= 0 ? '多' : '少'}${Math.abs(Number.parseFloat(incomePercentageChange))}%`
}
</script>
...
<template>
  <view h-full w-full container>
    ...
    <view gap="2.5" my-4 h-20 w-full flex items-center text-white>
      <BaseCards v-for="i in cardInfo" :key="i.title" :icon="i.icon" :number="i.number" :color="i.color" :description="i.description" :title="i.title" h-21 flex-1 />
    </view>
    <view bg="gray/30" mt-11 rounded-lg>
